...
Your task is to analyze hand-drawn 2D sketches, doodles, or wireframes and generate clean, beautiful Three.js 3D model creation code representing them.
...
3. Group all parts logically into a single root THREE.Group() (e.g. const group = new THREE.Group();).
...
6. DO NOT create any scene, camera, renderer, OrbitControls, animation loop, or HTML. THREE is already imported and available as a global.
7. The code MUST end with: return group;
8. Structure the code like this example:
```javascript
const group = new THREE.Group();
// Example: body
const bodyGeo = new THREE.BoxGeometry(2, 1, 1);
const bodyMat = new THREE.MeshStandardMaterial({ color: 0x3b82f6, roughness: 0.3, metalness: 0.1 });
...
return group;
```
9. Return ONLY the raw JavaScript code inside a ```javascript ... ``` code block. No HTML, no imports, no renderer, no scene creation.
"""
...
4. The code must end with `return group;` (returning the main THREE.Group).
5. DO NOT include scene, camera, renderer, OrbitControls, animation loop, or HTML. THREE is available as a global.
6. Return ONLY the updated raw JavaScript code inside a ```javascript ... ``` code block.
"""
